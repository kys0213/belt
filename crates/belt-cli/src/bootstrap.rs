...
pub async fn run_with_llm_interactive(
    ...
    let pr_url = if create_pr && !written.is_empty() {
        match create_bootstrap_pr(workspace_root, project_info) {
            Ok(url) => {
...
///
/// The function creates a timestamped branch (`bootstrap/<epoch>`), commits the
/// generated `.claude/rules/` files, pushes to the remote, and opens a PR
/// targeting `main`. If any step fails the original branch is restored.
///
/// Returns the PR URL on success.
fn create_bootstrap_pr(
    workspace_root: &Path,
    project_info: &ProjectInfo,
) -> anyhow::Result<String> {
    // Remember the current branch so we can restore on failure.
    let original_branch = get_current_branch(workspace_root)?;
    ...
    // Helper: restore the original branch on error.
    let restore_branch = |err: anyhow::Error| -> anyhow::Error {
        let _ = run_git(workspace_root, &["checkout", &original_branch]);
        err
    };

    // Create and checkout a new branch.
    run_git(workspace_root, &["checkout", "-b", &branch_name]).map_err(restore_branch)?;

    // Stage the convention files.
    run_git(workspace_root, &["add", ".claude/rules/"]).map_err(restore_branch)?;

    // Commit.
    let commit_msg = format!(
        "chore(bootstrap): add convention rules for {}\n\n\
         Generated by `belt bootstrap --llm`.",
        project_info.name
    );
    run_git(workspace_root, &["commit", "-m", &commit_msg]).map_err(restore_branch)?;

    // Push the branch.
    run_git(workspace_root, &["push", "-u", "origin", &branch_name]).map_err(restore_branch)?;
    ...
    let pr_body = format!(
        "## Summary\n\n\
         Add project-specific convention rules under `.claude/rules/`.\n\n\
         - Project: **{}**\n\
         - Language: {}\n\
         - Framework: {}\n\n\
         Generated by `belt bootstrap --llm --create-pr`.",
        project_info.name, project_info.language, project_info.framework,
    );

    // Create PR via gh CLI targeting main.
...
fn get_current_branch(dir: &Path) -> anyhow::Result<String> {
...
# Testing Content";
    ...
    #[test]
    fn get_current_branch_in_git_repo() {
        // This test runs inside the belt repo, so it should return a valid branch.
        let cwd = std::env::current_dir().unwrap();
        let result = get_current_branch(&cwd);
        // In a worktree or repo, this should succeed.
        assert!(result.is_ok(), "should detect current branch: {:?}", result);
        let branch = result.unwrap();
        assert!(!branch.is_empty(), "branch name should not be empty");
    }

    #[test]
    fn get_current_branch_fails_outside_git_repo() {
        let tmp = tempfile::tempdir().unwrap();
    ...
    #[test]
    fn create_bootstrap_pr_fails_outside_git_repo() {
        let tmp = tempfile::tempdir().unwrap();
        let info = ProjectInfo {
            name: "test-project".to_string(),
            language: "Rust".to_string(),
        ...
        let result = create_bootstrap_pr(tmp.path(), &info);
        assert!(
            result.is_err(),
    ...
    #[tokio::test]
    async fn interactive_bootstrap_no_pr_when_no_files_written() {
        let tmp = tempfile::tempdir().unwrap();

        // Pre-create files so nothing new is written.
        let rules_dir = tmp.path().join(".claude/rules");
        fs::create_dir_all(&rules_dir).unwrap();
        for name in &["project.md", "coding.md", "commit.md", "testing.md"] {
            fs::write(rules_dir.join(name), "# existing").unwrap();
        }
...
# New testing";
        ...
        // create_pr=true but no files written (all skipped) -> no PR should be created.
        let result = run_with_llm_interactive(
            tmp.path(),
            false,
            runtime,
        ...
        assert!(result.written.is_empty());
        assert_eq!(result.skipped.len(), 4);
        // No PR because nothing was written.
        assert!(
            result.pr_url.is_none(),
            "should not create PR when no files were written"
        );
    }
}
