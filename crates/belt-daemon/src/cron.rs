...
"#,
    ...
    fn gap_detection_authorization_middleware_with_high_threshold() {
        ...
        // Functions contain real auth logic so the gap detector only flags the
        // missing "endpoints" keyword, not empty bodies.
        std::fs::write(
            tmp.path().join("auth_mw.rs"),
            concat!(
                "use std::collections::HashMap;\n",
                "\n",
                "/// Check authorization by validating the bearer token against\n",
                "/// a set of known credentials and returning the associated role.\n",
                ...
                "}\n",
                "\n",
                "/// Implement the token refresh flow so callers can obtain new\n",
                "/// credentials without re-authenticating from scratch.\n",
                "fn implement_token_refresh(old_token: &str) -> Option<String> {\n",
                "    if old_token.is_empty() {\n",
                "        return None;\n",
                "    }\n",
                "    Some(format!(\"refreshed-{}\", old_token))\n",
                "}\n",
            ),
        )
        .unwrap();
