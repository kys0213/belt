...
- 디렉토리 자체가 없는 경우: agent는 built-in Agent rules(대화 턴 제한, 응답 포맷, 에러 핸들링)만으로 실행. 에러 없음.
...
- `crates/belt-cli/src/agent/mod.rs` — `AgentWorkspace::init`, `default_classify_policy()`
...
- [DESIGN-v6](../DESIGN.md) — QueuePhase 상태 머신 + evaluate 위치
