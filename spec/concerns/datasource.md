...
```bash
# hook.on_fail() script에서 실패 횟수 조회
...
handler 배열은 Running 상태에서 순차 실행. 하나라도 실패 시 hook.on_fail() → escalation.
...
```yaml
escalation:
  1: retry                # 같은 state에서 재시도 (hook.on_fail() 실행 안 함)
  2: retry_with_comment   # hook.on_fail() 실행 + 재시도
  3: hitl                 # hook.on_fail() 실행 + HITL 이벤트 생성
...
`retry`만 hook.on_fail()을 실행하지 않는다. 나머지(`retry_with_comment`, `hitl`)는 hook.on_fail() 실행 후 해당 액션을 수행한다.
