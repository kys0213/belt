...
**판정 기준은 룰베이스가 아닌 LLM 판단에 위임한다.** Belt가 처리하는 작업은 버그 수정, 기능 구현, 리팩토링, 문서 작성 등 맥락이 매우 다양하다. 획일적 규칙으로는 "충분한 결과"를 정의할 수 없으므로, LLM에게 관련 작업 맥락을 전달하여 판단하게 한다.

LLM에게 전달되는 작업 맥락:
- **이슈 원문**: DataSource에서 수집한 원본 이슈/요청 내용
- **handler 출력**: 코드 변경 diff, 실행 로그, 테스트 결과
- **이력**: 이전 시도 기록, 실패 사유, lateral plan (있는 경우)
- **classify-policy.md**: workspace별 판정 가이드라인 (운영자가 자연어로 작성)

classify-policy.md는 운영자가 해당 workspace의 품질 기대치를 자연어로 서술하는 문서이다. 예: "테스트가 통과하고 PR description이 작성되어 있으면 Done", "보안 관련 변경은 반드시 HITL". 이 문서가 없으면 LLM이 일반적 소프트웨어 엔지니어링 기준으로 판정한다.
...
impl EvaluationStage for SemanticStage {
    async fn evaluate(&self, ctx: &EvalContext, db: &dyn Db) -> Result<EvalDecision> {
        // belt agent -p 호출
        // ctx에 이슈 원문, handler 출력, 이력, classify-policy.md를 포함
        // LLM이 맥락을 종합 판단하여 belt queue done/hitl CLI를 호출
    }
}
```
